...
interface PainterState {
  ...
  /** Mirror of the tree's props so we can merge successive emissions
   *  (root-trace contributes nodes/edges; why-chain-deepened
   *  contributes driver counts) without losing fields. */
...
// ── Camera orchestration (P0 #2) ────────────────────────────────────
//
// Collect every shape the painter is actively managing and fit the
// viewport around them with a 600ms animated zoom. tldraw's
// zoomToBounds handles both pan and zoom in one call, which reads as a
// single cinematic sweep rather than two discrete operations.
//
// Debounced via state.cameraFitTimer so a burst of stage_boundary
// events (enter → exit → next enter in quick succession) coalesces
// into one final fit rather than fighting itself.
function fitViewportToUnfurl(editor: Editor, state: PainterState) {
