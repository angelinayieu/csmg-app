...
export type LayerStackShape = TLBaseShape<
  "layer-stack",
  {
    ...
    layersJson: string;
    /**
     * D-track Phase β — JSON-stringified Array<{
     *   from_layer_id, to_layer_id, kind, strength,
     *   lag_label, mechanism
     * }> from `layer_dependencies`. Empty array `"[]"` when the cascade
     * prediction hasn't run yet for this space (legacy spaces / pre-D-track
     * snapshots). When present, the shape renders inter-layer cascade
     * arrows on the right edge of the stack, color-coded by `kind`.
     */
...
export type ForestPlotShape = TLBaseShape<
  "forest-plot",
  {
    ...
    findingCount: number;
    /**
     * Phase E focus mode. JSON-encoded `string[]` of `ForestFinding.id`
     * values to highlight (full opacity + 2px teal left rail). When the
     * array is non-empty, all other rows are rendered at 0.30 opacity so
     * the user can still read the surrounding context. Default `"[]"`
     * (no focus — all rows opaque). Stored as JSON string for the same
     * reason `findingsJson` is: tldraw's flat-prop serializer.
     */
    highlightedFindingIds: string;
    /**
     * Phase E: human-readable label for the focused edge (e.g.,
     * "supports · behavior") surfaced in the focus chip at the top-right
     * of the card. `null` when no focus is applied.
     */
    focusedEdgeLabel: string | null;
    /**
     * Phase 0 rigor: which tier filter is active. "anchored" shows
     * only paper-sourced findings (default — hallucinated effect
     * sizes can never silently outrank reviewed ones). "all" reveals
     * every tier with drifted rows visually de-emphasized. Defaults
     * to "anchored" so the safe behavior is the path of least
     * resistance.
     */
    tierMode: "anchored" | "all";
    /**
     * Phase 0 rigor: per-tier counts across the FULL eligible pool
     * (BEFORE the active tier filter slices it down). JSON-encoded
     * `Record<RigorTier, number>` for the same reason `findingsJson`
     * is JSON-string: tldraw's flat-prop serializer rejects nested
     * objects. The shape uses this to render the footer breakdown
     * ("4 anchored · 2 drifted") regardless of which mode is active.
     */
    tierCountsJson: string;
  }
>;
...
// Reflexive pulse — the ambient state of the prospector / coverage /
// calibration subsystems, surfaced inside the Reflexive room. Renders
// two compact bars (coverage % + depth %) + a calibration line + a
// "Scan now" affordance. Polls /api/spaces/[id]/intelligence-overview
// every 30s while visible; explicit-only manual scan. Apple-quiet
// glance, NOT a dashboard — the full intelligence page handles the
// inbox + dimension breakdowns.
//
// All numeric props are derived in the spawner from the
// IntelligenceOverview payload + space entity count. Mutating any
// prop on the shape triggers the view's standard re-render.
export type ReflexivePulseShape = TLBaseShape<
  "reflexive-pulse",
  {
    w: number;
    h: number;
    spaceId: string;
    /** Total pairwise checks performed. Numerator of coverage %. */
    totalChecks: number;
    /** n*(n-1)/2 over current entity count. Denominator of coverage %. */
    totalPossiblePairs: number;
    /** Checks at examination_level >= 2 (dimensional sweep or deeper). */
    levelTwoPlus: number;
    /** Acceptance rate over last 30 days of user_verdict rows. Null
     *  when the user has reviewed fewer than the awaiting-threshold (3). */
    acceptancePct: number | null;
    /** Count of user-reviewed verdicts in the last 30 days — drives the
     *  "awaiting reviews vs %" tone choice. */
    verdictCount: number;
    /** Proposed-edges queue length (drives the "Open inbox · N new" link). */
    proposedEdgesCount: number;
    /** ISO timestamp of the most recent pairwise check. Null = never scanned. */
    lastCheckedAt: string | null;
    /** Bumped on scan-complete so the view can fire a 8s delta banner. */
    pulse: number;
    /** Delta payload from the last successful scan, surfaced in the
     *  inline banner. Cleared on next poll. */
    lastScanDelta: string | null;
    /** True while a manual "Scan now" POST is in flight. View shows
     *  spinner + dims the metric bars to 60%. */
    scanning: boolean;
  }
>;

// P1.1 (operational whiteboard) — Learning-loop card. Surfaces the
// strategy's StrategyLearningLoop (lagging_indicator + leading_indicators +
// review_cadence + pivot_criteria + persistence_signals) as a canvas
// shape inside the twin room. This is the operational dashboard the user
// reads post-approval — "what do I watch, and when do I pivot?"
//
// Today learning_loop is generated by the strategy LLM but only surfaces
// inside the off-canvas strategy-page's whiteboard-view
// (build-whiteboard-model.ts:422-459). This shape brings it onto the
// actual tldraw canvas so users see it alongside the twin-snapshot,
// closing the most operationally valuable data gap on the canvas.
export type LearningLoopCardShape = TLBaseShape<
  "learning-loop-card",
  {
    w: number;
    h: number;
    spaceId: string;
    /** ISO timestamp when the card was first spawned. */
    capturedAt: string;
    /** The aspiration metric the strategy is gunning for. */
    laggingMetric: string;
    laggingTarget: string;
    laggingDeadline: string;
    /**
     * JSON-stringified array of up to 4 leading indicators:
     * [{ metric, cadence, green_reading, yellow_reading, red_reading,
     *    measurement_method, status }]
     * Stored as JSON for tldraw prop-typing simplicity (matches the
     * synthesis-intersection-card's rowsJson pattern at
     * synthesis-intersection-card-shape.tsx).
     */
    leadingIndicatorsJson: string;
    /** Cadence the strategy commits to for metric checks (e.g., "daily"). */
    metricCheckCadence: string;
    /** Full strategy review cadence (e.g., "biweekly"). */
    reviewCadence: string;
    /** Target cycles in the timeline (LLM-set). */
    totalCycles: number;
    /**
     * JSON-stringified array of pivot criteria:
     * [{ signal, response, specific_action, timeline }]
     */
    pivotCriteriaJson: string;
    /** Count of persistence signals — footer chip rather than full
     *  render to preserve card real estate. */
    persistenceCount: number;
    /** Hex accent color — drives the header rail + indicator chip tints. */
    accent: string;
  }
>;
...
export type StrategyHeroCardShape = TLBaseShape<
  "strategy-hero-card",
  {
    ...
    pulse: number;
    /**
     * E3 — layer-stratified variant marker. When the active rank was
     * generated with a single-layer focus, these three fields drive the
     * "● <Layer>" chip in the card header + the left-border accent
     * stripe. All three null for comprehensive variants (no chip
     * renders). Sourced from `StrategicRecommendation.layer_focus`.
     */
    activeLayerFocusId: string | null;
    activeLayerFocusLabel: string | null;
    activeLayerFocusColor: string | null;
  }
>;
...
export type AppCardShape = TLBaseShape<
  "app-card",
  {
    ...
    name: string;
    /**
     * One-line "what this app actually does for you." Sourced from
     * apps.config.tagline at drop time. Renders below the title in
     * muted color (2-line clamp). Replaces the prior health-score
     * readout as the card's primary utility signal — `health %` was
     * a stale-after-generation number that read as alive-stats theater;
     * tagline directly answers "what is this thing for?"
     *
     * Empty string when the strategy didn't ship a tagline; the view
     * collapses the line in that case so the card layout stays tight.
     */
    tagline: string;
    appType: AppType;
    status: AppStatus;
    /**
     * 0..100 — null when not computed. RETAINED for back-compat with
     * persisted shapes that carry this prop, but NO LONGER RENDERED on
     * the card (was deceptive — health was set at app-gen time and
     * never recomputed). When the continuous-improvement loop ships
     * (agents writing back signals), this becomes a real signal again
     * and the view can re-enable rendering.
     */
