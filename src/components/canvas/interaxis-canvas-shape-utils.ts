...
export const SHAPE_UTILS = [
  ...
  // C1 — reflexive-pulse: ambient glance at prospector / coverage /
  // calibration inside the Reflexive room. Polls intelligence-overview
  // every 30s while visible; Scan-now triggers a prospector batch.
  ...
  // P1.1 — Learning-loop card inside the twin room. Surfaces the
  // strategy's StrategyLearningLoop (lagging_indicator + leading_indicators
  // + review_cadence + pivot_criteria + persistence_signals) — the
  // operational dashboard the user reads post-approval. Until this
  // landed the data was generated by the LLM but rendered nowhere
  // on the canvas (only inside the off-canvas strategy-page's
  // whiteboard-view at build-whiteboard-model.ts:422-459).
