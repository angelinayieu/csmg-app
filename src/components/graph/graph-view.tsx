...
export function GraphView() {
  ...
  const filteredEdges = useMemo(() => {
    // Edges reference entities by UUID (edges.source_entity_id / target_entity_id
    // store the Postgres UUID — see sanitizeEdge). Previously this set was
    // built from e.entity_id (the semantic "C1"/"X2" short IDs), which
    // produced a 100% filter-out rate and caused the "No connections between
    // entities" banner to fire on any space whose dedicated /graph route was
    // opened — even when edges existed in the DB and rendered correctly in
    // the sidebar / tabs view.
    const entityUuids = new Set(filteredEntities.map((e) => e.id));
    const allEdges = showSiblings ? [...ctx.edges, ...ctx.siblingEdges] : ctx.edges;
    return allEdges.filter(
      (e) =>
        entityUuids.has(e.source_entity_id) &&
        entityUuids.has(e.target_entity_id) &&
  ...
  // Quick Connect — remediates the "no connections" warning inline without
  // forcing the user to navigate back to the space header.
  const quickConnect = useQuickConnect(ctx.space.id);
