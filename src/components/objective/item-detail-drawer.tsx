...
// ── Expansion Panel ───────────────────────────────────────────────
//
// The [+] Deepen control + inline-render of L3+ children for a
// variation (or any other attach point in future). Lives at the
// bottom of every variation card; collapsed-by-default to keep the
// drawer readable until the user actually wants depth.
//
// On first click:
//   • If children already exist in tree → just expand the view.
//   • Otherwise fire POST /api/brainstorm/item/expansion/spawn with
//     the parent attach, append the returned children to the tree.
...
function ExpansionPanel({
  ...
  expansionTree: ExpansionNodeLocal[];
  onTreeUpdate: (next: ExpansionNodeLocal[]) => void;
  /** Visual nesting hint — caps the recursion at depth 3 so a runaway
   *  tree doesn't render off-screen. The route already gates by
   *  catalog availability, so this is a defense-in-depth limit. */
  depth?: number;
}) {
  ...
  // Children = expansion_tree rows that hang off THIS exact attach.
  // For L3, that's (parent_node_id === null && attach_point/ref match).
  // For L4+, that's (parent_node_id === attachRef).
  const isExpansionNodeParent = attachPoint === "expansion_node";
  const children = useMemo(() => {
    return expansionTree.filter((n) => {
      if (isExpansionNodeParent) return n.parent_node_id === attachRef;
      return (
        n.parent_node_id === null &&
        n.attach_point === attachPoint &&
        n.attach_ref === attachRef
  ...
  }, [expansionTree, attachPoint, attachRef, isExpansionNodeParent]);
  ...
  const fireSpawn = useCallback(
    async (force = false) => {
      ...
      try {
        const res = await fetch("/api/brainstorm/item/expansion/spawn", {
          ...
          body: JSON.stringify({
            entityId,
            parentNodeId: isExpansionNodeParent ? attachRef : null,
            attachPoint: isExpansionNodeParent ? "expansion_node" : attachPoint,
            attachRef,
            parentTitle,
            parentDescription,
            mode: force ? "force" : "default",
        ...
        const json = await res.json();
        if (!res.ok) {
          setErr(json?.error ?? "Expansion failed.");
          return;
        }
        if (Array.isArray(json?.tree)) {
          onTreeUpdate(json.tree as ExpansionNodeLocal[]);
    ...
    [entityId, attachPoint, attachRef, parentTitle, parentDescription, isExpansionNodeParent, onTreeUpdate],
  );

  // Recursion guard — don't nest forever in the drawer (canvas
  // escape hatch will eventually carry depth >3).
  const allowFurtherDepth = depth < 2;
    ...
    return (
      <div className="mt-2">
        <button
          ...
          title="Deepen this variation into technical depth surfaces (mechanism, data model, edge cases, telemetry, rollback)"
        >
          {busy ? (
            <>
  ...
  return (
    <div className="mt-2">
      <div className="flex items-center justify-between">
        ...
        >
          {open ? "▾" : "▸"} Deep dive · {children.length}{" "}
          {children.length === 1 ? "surface" : "surfaces"}
        </button>
        {open && (
          <button
            type="button"
            ...
            title="Regenerate this expansion"
          >
            <RefreshCw
        ...
        >
          {children.map((child) => (
            <ExpansionNodeCard
              ...
              expansionTree={expansionTree}
              onTreeUpdate={onTreeUpdate}
              allowFurtherDepth={allowFurtherDepth}
              depth={depth + 1}
            />
          ))}
        </ul>
...
// ── Expansion Node card ────────────────────────────────────────────
//
// Renders a single L3+ node with its breadcrumb path, body content,
// and (when the catalog allows further depth) a nested ExpansionPanel
// for L4 children.
...
function ExpansionNodeCard({
  ...
  expansionTree: ExpansionNodeLocal[];
  onTreeUpdate: (next: ExpansionNodeLocal[]) => void;
  allowFurtherDepth: boolean;
  depth: number;
}) {
  // Breadcrumb shows the lineage_titles + the node's own title at
  // the end. Only render the breadcrumb when there's actual lineage
  // (i.e. depth > L3, since L3's lineage is just the variation name
  // which is already visible above).
  const showBreadcrumb = node.lineage_titles.length > 1;
  ...
  return (
    ...
    >
      {showBreadcrumb && (
        <div
          className="mb-1 text-[9.5px] font-light"
          style={{ color: appleVibe.text.tertiary }}
        >
          {node.lineage_titles.join(" › ")}
        </div>
      )}
      <div className="flex items-center justify-between gap-2">
        <div
        ...
        <span
          className="text-[9px] font-semibold uppercase tracking-[0.12em]"
          style={{ color: appleVibe.text.tertiary }}
          title={`Layer ${node.depth}`}
        >
          L{node.depth}
        </span>
      </div>

      <ExpansionBody body={node.body} nodeType={node.node_type} />

      {/* Annotation provenance chip — when the LLM tagged this node. */}
      {node.derived_from_annotations && node.derived_from_annotations.length > 0 && (
        <div className="mt-1.5 flex flex-wrap items-center gap-1">
          {node.derived_from_annotations.map((p) => (
            <span
              key={`${p.index}-${p.facet}`}
              className="inline-flex max-w-[140px] items-center gap-1 rounded-full px-1.5 py-0.5 text-[9px] font-medium"
              style={{
                background: "rgba(15,23,42,0.035)",
                color: appleVibe.text.tertiary,
                border: `1px solid ${appleVibe.stroke.hairline}`,
      ...
      {/* Recursive [+] Deepen for L4 children. Only render when the
          drawer permits further depth + the catalog would have an
          entry (the route 409s gracefully if not). */}
      {allowFurtherDepth && (
        <ExpansionPanel
          entityId={entityId}
          attachPoint="expansion_node"
          attachRef={node.id}
          parentTitle={node.title}
          parentDescription={JSON.stringify(node.body).slice(0, 300)}
          expansionTree={expansionTree}
          onTreeUpdate={onTreeUpdate}
          depth={depth}
        />
...
// ── Body renderer ──────────────────────────────────────────────────
//
// Catalog-aware-ish: we look at the body shape (which keys are
// present) and render accordingly. New node_types just add new
// cases — fallback dumps prettified JSON so the surface never
// blanks even when the LLM emits an unknown shape.
...
function ExpansionBody({
  ...
  // Single-paragraph nodes (mechanism_story, etc.)
  ...
  // Software data model
  if (nodeType === "software.data_model") {
    const entities = Array.isArray(body.entities) ? body.entities : [];
    const relations =
      typeof body.relations_summary === "string"
        ? body.relations_summary
        : "";
    return (
      <div className="mt-1.5 flex flex-col gap-1.5">
        {entities.map((e, i) => {
          ...
          return (
            <div key={i} className="text-[10.5px] font-light leading-snug">
              <span
              ...
              {typeof r.notes === "string" && r.notes.length > 0 && (
                <div
  ...
  // Software edge cases
  if (nodeType === "software.edge_cases") {
          ...
          return (
            ...
            >
              <span style={{ color: appleVibe.text.primary }}>
                {typeof r.scenario === "string" ? r.scenario : ""}
              </span>
              {typeof r.handling === "string" && r.handling.length > 0 && (
  ...
  // Software telemetry
  if (nodeType === "software.telemetry") {
    const events = Array.isArray(body.events) ? body.events : [];
    const antiMetric =
      typeof body.anti_metric === "string" ? body.anti_metric : "";
    return (
      <div className="mt-1.5 flex flex-col gap-1">
        ...
        {antiMetric.length > 0 && (
          <div
            className="mt-1 rounded px-1.5 py-0.5 text-[10px] font-light"
            style={{
              background: "rgba(220,38,38,0.06)",
              border: "1px solid rgba(220,38,38,0.18)",
              color: "rgba(127,29,29,0.95)",
  ...
  // Software rollback
  if (nodeType === "software.rollback_strategy") {
    return (
      <div className="mt-1.5 flex flex-col gap-1 text-[10.5px] font-light leading-snug">
        {typeof body.kill_switch === "string" && (
          <div>
            <span className="font-semibold" style={{ color: appleVibe.text.primary }}>
              kill switch:
            </span>{" "}
            <span style={{ color: appleVibe.text.secondary }}>{body.kill_switch}</span>
          </div>
        )}
        {typeof body.data_safety === "string" && (
          <div>
            <span className="font-semibold" style={{ color: appleVibe.text.primary }}>
              data safety:
            </span>{" "}
            <span style={{ color: appleVibe.text.secondary }}>{body.data_safety}</span>
          </div>
        )}
        {typeof body.recovery === "string" && (
          <div>
            ...
            <span style={{ color: appleVibe.text.secondary }}>{body.recovery}</span>
          </div>
        )}
      </div>
    );
  ...
  // Resolution options (conflict_open expansion)
  if (nodeType === "software.resolution_option") {
  ...
  // Pointers (adjacent evidence)
  if (nodeType === "software.adjacent_evidence") {
      ...
      <ul className="mt-1.5 list-disc space-y-1 pl-4">
        {pointers.map((p, i) => {
          ...
          return (
            ...
            >
              <span className="font-semibold" style={{ color: appleVibe.text.primary }}>
                {typeof r.source === "string" ? r.source : "source"}
              </span>
  ...
  // Generic fallback — render any { key: string | string[] } pairs.
        ...
        if (typeof v === "string") {
          return (
            <div key={k}>
              ...
              <span style={{ color: appleVibe.text.secondary }}>{v}</span>
            </div>
          );
        }
        if (Array.isArray(v)) {
          return (
            <div key={k}>
              ...
              <ul className="ml-3 list-disc space-y-0.5 pl-4">
                {v.map((item, i) => (
                  <li
                    key={i}
                    style={{ color: appleVibe.text.secondary }}
