...
export interface ItemVariation {
  ...
  /** Phase 11.2 — per-proxy-indicator grades for this variation. Populated
   *  by the rubric scorer when the target room's outcomes carry
   *  causal_chain.indicators[]. Each entry is the verbatim indicator
   *  text + a 0..1 score for "how much does this variation move THIS
   *  specific indicator" + a one-sentence reason + a 0..1 confidence
   *  for "how well does this proxy actually map to the outcome we
   *  care about."
   *
   *  Why separate from rubric_criteria: the 5 generic criteria
   *  (plausibility/addresses_pain/...) compose into the headline
   *  composite_score (and stay the same shape for all variations on a
   *  feature). Indicator scores are SUPPLEMENTARY — they reveal which
   *  proxies a variation moves vs which it doesn't, addressing the
   *  user's stated requirement: "scoring based on proxy indicators."
   *  Future: indicator confidence could re-weight the composite.
   *
   *  Undefined when the target outcomes have no indicators, the
   *  scorer skipped them (e.g., method="simulation"), or the LLM
   *  failed on that side of the prompt. */
  indicator_scores?: Array<{
    /** Verbatim indicator text from outcome.causal_chain.indicators[]. */
    indicator_text: string;
    /** Which outcome this indicator belongs to. */
    outcome_id: string;
    outcome_name: string;
    /** 0..1 — how much does THIS variation move THIS indicator. */
    score: number;
    /** One sentence — why this score. */
    reason: string;
    /** 0..1 — confidence that this proxy indicator actually maps to
  ...
  /** Op C — separate thumbnail-format mockup (480×320, single screen)
   *  for inline rendering below an expanded variation row on the
   *  CategoryCard. Persisted alongside the fullscreen version so the
   *  user can flip between them without re-spending an LLM call. */
  mockup_thumbnail_html?: string;
  mockup_thumbnail_generated_at?: string;
  ...
  /** Op A — PR/FAQ-style description doc (5 headings, ~400 words).
   *  Distinct from `description` (1-2 sentences) and `export_prompt`
   *  (framed for another LLM). Surfaces in the Deliverables modal's
   *  Doc tab + inline below the mockup on expanded mechanism rows. */
  description_doc?: string;
  description_doc_generated_at?: string;
  /** Op B — extended export-prompt shape when the round-trip
   *  optimizer ran. Carries v1 + judge verdict + (optional) v2 +
   *  preview outputs. Co-exists with export_prompt (final) for
   *  backward compat. */
