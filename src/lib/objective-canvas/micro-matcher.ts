...
//
// Address vs resolve: this layer ONLY writes `addressed_by[]` entries (the
// gradient — "work touches this sub-question"). Flipping a micro to fully
// resolved stays an explicit gesture (Resolution Studio / Let AI decide / a
// future high-confidence threshold). Tier-0 makes the rail breathe; it
// deliberately doesn't make trust calls.
//
...
export function matchMicros(
  ...
  for (const ann of anns) {
    ...
    for (const m of micros) {
      ...
      const microToks = tokenize(m.q);
      // Inject the macro's distinctive slug parts so a card that just mentions
      // the concept by name still counts as touching its sub-questions.
      ...
      for (const c of tokenizedCandidates) {
        const refKey = `${c.kind}:${c.ref}`;
        if (existingRefs.has(refKey)) continue;
        const score = recall(microToks, c.toks);
        if (score < ADDRESS_THRESHOLD) continue;
        matches.push({
          ref: { kind: c.kind, ref: c.ref, at: nowIso },
          score,
        });
      }
      ...
      out.push({
        microId: m.id,
        newRefs: matches.slice(0, room).map((m2) => m2.ref),
      });
    }
  }
  return out;
...
export function applyMicroAddressing(
  ...
  const byMicroId = new Map<string, AddressingRef[]>();
  for (const d of deltas) byMicroId.set(d.microId, d.newRefs);
  return anns.map((a) => {
    const micros = a.micro_questions;
    if (!micros || micros.length === 0) return a;
    let touched = false;
    const next = micros.map((m) => {
      const add = byMicroId.get(m.id);
      if (!add || add.length === 0) return m;
      touched = true;
      return {
        ...m,
    ...
    return touched ? { ...a, micro_questions: next } : a;
  });
}
