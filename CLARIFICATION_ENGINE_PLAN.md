...
3. **Start at Step 1** (wire the inert micro half) — ✅ DONE 2026-06-05 (tsc-clean): `liveUncertainty()` helper + matcher auto-resolve (strict own-token ≥0.75) + `applyMicroAddressing` recomputes/persists macro uncertainty + rail 3-state dots (open/addressing/resolved).
   ...
   **Remaining:** Step 3 (planner / intelligent op-selection — today the op is always resolve→decompose) + Step 5 FULL (recursive diverge/converge/self-question + KG/glossary build) + Step 7 FULL (wire the approved look onto a real custom tldraw connector shape over the native-arrow binding + the board-scoped dark theme). Larger, genuinely-new builds.
