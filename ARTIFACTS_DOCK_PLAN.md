...
>
> **Phase 3A (Notebook rebuild — on-canvas panel) DONE.** Block model `notebook-types.ts` (ai_woven|user|quote; coerce + legacy-journal backfill). Append-only generator `generate-notebook.ts` (`synthesizeNotebookForSpace` weaves ONLY notes past `wovenCount`, appends `quotes` from selected cards, never rewrites user/quote/locked blocks). `run_notebook` route rewired to it. Editable panel `notebook-panel.tsx` (`NotebookMount`, opens on `OPEN_NOTEBOOK_EVENT`): per-block edit/add/lock/delete, "Weave new notes", autosave on close, refreshes the board card. journal-card "Open" → opens the panel (booklet flip retired/dead). Dock Notebook engine passes selected cards as quote blocks + opens the panel. **Phase 3B (full-page `/notebook/[artifactId]` route) NOT built** — panel only so far.
