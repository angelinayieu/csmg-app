...
→ **The connection graph's REQUIRED substrate is these declared bindings** (features linked by shared `root_cause`, and by `outcome → pain` matches); mechanism tokens + registry are an **OPTIONAL enrichment** that deepens an edge with a precise data-unit label + operators *when a spec exists*.

**(A′) Canonical identity — use `canonical_concept_id`, NOT `concept_slug`.**
KG-wiring audit: `concept_slug` is a *derived JSONB string* that only joins **annotations ↔ glossary** (`synthesis_data.glossary`); it is **never a DB column and never written on `entities`** — so keying connections on it would be a new orphan. The real, persisted, cross-space concept identity is **`entities.canonical_concept_id` → the `canonical_concepts` table** (`20260721`), matched by `linkEntityToCanonicalConcept` (`canonical-concept-matcher.ts`). **But it is sparse:** only 3 producers populate it (framing-protos / decompose / variable-approve) — **none in room/feature generation**, so today features/pains/outcomes carry no canonical id and cross-room identity falls back to verbatim string match (`cross-room-signals.ts`). → **The true base-infra move is P0 below: populate `canonical_concept_id` during room generation.** Until an entity is linked, fall back to normalized free-text (`slugifyConcept()` over the verbatim `addresses/moves` strings — what the system de-facto runs on). This wires the map into the *real* KG + glossary instead of a parallel identity. §2 revised accordingly.

**(B) Viz stack — React Flow + ELK (elkjs), not dagre.**
Research verdict: keep `@xyflow/react` (right renderer — custom operator/data-unit nodes, the complexity dial, and hover-trace are all pure React state), but adopt **ELK (`elkjs`) `layered`** for layout: orthogonal/Manhattan edge routing, barycenter crossing-minimization (LAYER_SWEEP), Brandes-Köpf node placement, and **reserved edge-label space** — none of which dagre does (that's the Lucidchart/swimlane look the reference images have). L1→L4 = ELK layers; render lanes as non-interactive **background bands**. Hold `d3-sankey` in reserve for a separate data-**VOLUME** sub-mode only. It's an **additive `elkjs` install**, no rewrite. (React Flow ships an official ELK example.) §3 is revised accordingly. (`elkjs` is NOT yet installed; only `@dagrejs/dagre` is.)

**(C) VERIFIED against the live DB 2026-05-30 — the substrate is thinner + differently-shaped than §1–§2 assumed. THREE corrections (supersede §0A's field names + §2's token-centric builder):**
1. **`causal_chain` shape is different.** OC room entities (510, identified by `parent_sub_objective_id NOT NULL`) actually store `{ root_causes[]:string, negative_outcome:string, sub_category, influence_rank, derived_from_annotations[] }` — **NOT** the `addresses[]`/`moves[]` (`{pain,root_cause}`,`{outcome,indicator}`) §0A/§2 keyed on. Those keys exist on only **78/510**. The reliably-populated binding (**400/510, 78%**) is **`root_causes[]` + `negative_outcome`**. A builder MUST key on these (+ `derived_from_annotations[].phrase`, which is the parallel chat's annotation substrate already landing on entities).
2. **Both enrichment + canonical join are near-empty today.** `mechanism_spec` tokens on **26/510**; `canonical_concept_id` on **0/510** (see MASTER plan P2). So §2a's "precise token graph" and the canonical key are both essentially absent on current data.
3. **The free-text string-match graph is too thin to ship.** Exact shared-`root_cause` cross-room pairs = **39, in just 2 spaces** (out of 510 entities). Match-only canonical backfill yield = **5/510** (rest would mint sentence-noise into the shared registry). → **The map CANNOT meet §6 acceptance ("no string-match edges") on current data.** P4 is empirically **downstream of P2**, not parallel to it.

**Consequence (build order):** do NOT build `SystemDataflowView` yet — on today's data it renders near-empty or falls back to the exact fuzzy match this spec exists to kill. **Unblocking prerequisite = P2**: populate `canonical_concept_id` **inline at room generation** (the matcher is pure string-normalization, cheap — `canonical-concept-matcher.ts:279`), NOT via a bulk backfill (505/510 would pollute the cross-space registry). Sequence: **P2 (inline canonical link) → rebuild rollup keyed on `canonical_concept_id` → `SystemDataflowView`.** P2's call-sites (`room/generate`, `run-feature`) are currently HOT (parallel "Subsystems re-altitude" session) — wire when they settle.

**(D) DELIVERED 2026-05-30 — the connection layer (clustering, not a new view).** `SystemDataflowView` is RETIRED (parallel session owns the views — `SUBSYSTEM_KG_SPEC.md`). The real gap was the *substrate*: the same concept recurs across rooms with VARIED phrasing, and exact `canonical_concept_id` keeps the variants distinct (6 exact cross-room links). **Discovery (live DB):** deterministic clustering (canonical + exact-name + token-Jaccard + substring) surfaces **~235 high-quality candidate cross-room links** (e.g. "Privacy Concerns with Data Sharing" ≈ "Data Privacy Concerns" ≈ "Privacy Concerns Hinder Data Sharing"). **Shipped:** pure `src/lib/objective-canvas/build-concept-clusters.ts` (transitive union-find; `scorePair` is the pluggable seam — swap for embedding cosine to go lexical→semantic) + read-only `GET /api/objective/[spaceId]/concept-connections` (loads room entities, returns `clusters` + cross-room `connections`). No writes, no LLM, no embeddings, no schema. **This SUPERSEDES `cross-room-signals` category-2 (verbatim root-cause match)** — the views can swap their cross-room edge source to this route for the dense map (parallel session's call; coordinate). **Next upgrade (your nod): generate concept embeddings → swap `scorePair` → catch synonyms sharing 0 tokens.**
